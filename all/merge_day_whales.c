...
void merge_day_whales(struct pod *first_pod) {
    ...
    while(p->next != NULL) {
        ...
        while (check->next != NULL) {
            if(strcmp(p->species, check->next->species) == 0 && p->when->day == check->next->when->day && p->when->month == check->next->when->month && p->when->year == check->next->when->year) {
                struct pod *n = check->next;
                p->how_many = p->how_many + check->next->how_many;
                check->next = n->next;
                free(n->when);
                free(n->species);
                free(n);
