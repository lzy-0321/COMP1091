...
void species_count(char species[], int n_sightings, struct pod sightings[n_sightings],
    ...
    // Check all elements in the array, of length n_sightings.
    while (position < n_sightings) {
        // Compare element in array to our species. If they are equal,
        // increment pod and whale_total
        if (strcmp(sightings[position].species, species) == 0) {
            pod_total++;
            whale_total = whale_total + sightings[position].how_many;
        }
        position++;
    }

    *n_pods = pod_total;
    *n_whales = whale_total;
}
