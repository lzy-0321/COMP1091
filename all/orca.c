...
// Compares array element with "Orca".
int compare(int n_sightings, struct pod sightings[n_sightings], int count, int i);
...
int count_orca_sightings(int n_sightings, struct pod sightings[n_sightings]) {
    ...
    int i = 0;
    while (i < n_sightings) {
        // Calls function to compare array element with "Orca".
        count = compare(n_sightings, sightings, count, i);
        i++;
    }
    return count;
}

// Compare the element of the array to Orca.
// If it is equal, then there is an orca sighting.
int compare(int n_sightings, struct pod sightings[n_sightings], int count, int i) {
